...
const getDashboardStats = async (startDate?: string, endDate?: string) => {
  ...
  // Build countDocuments filter — only apply date range when provided
  const userDateFilter = Object.keys(dateMatch).length > 0 ? dateMatch : {};
  ...
  const [
    ...
    // course breakdown by status — courses have no meaningful date filter here
    CourseModel.aggregate([
      {
        $group: {
          _id: null,
    ...
    // teachers — filter by createdAt when date range provided
    User.countDocuments({ role: Role.TEACHER, ...userDateFilter }),

    // students — filter by createdAt when date range provided
    User.countDocuments({ role: Role.STUDENT, ...userDateFilter }),

    // classes — filter by createdAt when date range provided
    ClassModel.countDocuments({ ...userDateFilter }),

    // subjects — filter by createdAt when date range provided
    SubjectModel.countDocuments({ ...userDateFilter }),
  ]);
...
const getStudentAnalytics = async (
  ...
  const userDateFilter = Object.keys(dateMatch).length > 0 ? dateMatch : {};
  const totalStudents = await User.countDocuments({ role: Role.STUDENT, ...userDateFilter });
  const activeStudents = await User.countDocuments({ role: Role.STUDENT, isActive: true, ...userDateFilter });
