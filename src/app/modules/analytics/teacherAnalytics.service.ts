...
//                   $and: [
//                     { $eq: ["$$item.teacher", tid] },
//                     { $eq: ["$$item.subject", "$subjectId"] },  // ← ZoomMeeting.subjectId
//                   ],
//                 }
//               },
//             },
...
const buildGroupByDate = (
  ...
  return {
    $dateToString: {
      format: formats[granularity] ?? formats.month,
      date: "$createdAt",
    },
  };
};
...
/**
 * A ZoomMeeting belongs to THIS teacher only when:
 *   ZoomMeeting.subjectId === assignSubWithTeacher[i].subject
 *   assignSubWithTeacher[i].teacher === teacherId
 * Both must match simultaneously.
 */
const teacherClassPipeline = (tid: Types.ObjectId) => [
  {
    $lookup: {
      from: "courses",
      localField: "courseId",
...
const getTeacherStats = async (
  ...
  const classResult = await ZoomMeeting.aggregate([
    { $match: { status: "COMPLETED", ...dateMatch } },
    ...teacherClassPipeline(tid),
    {
      $lookup: {
        from: "users",
        localField: "matchedAssign.teacher",
        foreignField: "_id",
        as: "teacherInfo",
      },
    },
...
// ─── 2. Teacher Salary Over Time (chart) ──────────────────────────────────────
...
// ─── 3. Per-Course: teacher salary + course payment revenue ───────────────────
//
// teacherSalary  = this teacher's classes in this course × perClassSalary
// courseRevenue  = total payments collected from students for this course
//
// Both shown separately so the table is unambiguous.
...
const getTeacherCourseRevenue = async (
  ...
  // ── teacher salary per course (classes × perClassSalary) ──
  const salaryPerCourse = await ZoomMeeting.aggregate([
    {
      $match: {
        status: "COMPLETED",
    ...
    { $unwind: "$teacherInfo" },
    {
      $group: {
        _id: "$courseId",
        totalClasses: { $sum: 1 },
        // ✅ teacher salary = sum of perClassSalary for THIS teacher's classes
        teacherSalary: { $sum: "$teacherInfo.perClassSalary" },
      },
    },
  ]);
  const salaryMap = new Map(
    salaryPerCourse.map((r: any) => [String(r._id), r])
  );

  // ── course payment revenue (what students paid) ──
  const paymentPerCourse = await PaymentModel.aggregate([
    { $match: { status: "COMPLETED", ...dateMatch } },
    {
      $lookup: {
        from: "enrollments",
  ...
  const paymentMap = new Map(
    paymentPerCourse.map((r: any) => [String(r._id), r])
  );

  // ── merge all assigned courses ──
  const courses = assignedCourses.map((c) => {
    const sal = salaryMap.get(String(c._id));
    const pay = paymentMap.get(String(c._id));
    return {
      courseId: String(c._id),
      courseTitle: (c as any).title,
      courseStatus: (c as any).status,
      totalClasses: sal?.totalClasses ?? 0,
      // ✅ what this teacher earned from this course
      teacherSalary: sal?.teacherSalary ?? 0,
      // ✅ what students paid for this course
      courseRevenue: pay?.courseRevenue ?? 0,
      totalStudents: pay?.totalStudents ?? 0,
    };
  });

  const summary = {
    totalCourses: courses.length,
    // salary summary for stat card
    totalTeacherSalary: courses.reduce((s, c) => s + c.teacherSalary, 0),
    // course revenue summary
    totalCourseRevenue: courses.reduce((s, c) => s + c.courseRevenue, 0),
    totalStudents: courses.reduce((s, c) => s + c.totalStudents, 0),
    totalClasses: courses.reduce((s, c) => s + c.totalClasses, 0),
  };
